...
Supports both Upstash Redis (REST API) and regular Redis.
"""
import json
import gzip
import hashlib
...
from app.models.requests import UnQuestRequest
import structlog
...
class CacheService:
    ...
    Supports both Upstash Redis (REST API) and regular Redis connections.
    Automatically uses Upstash when credentials are available.
    """
    ...
    async def initialize(self):
        """Initialize Redis connection (Upstash or regular Redis)."""
        if not self._client:
            if self._use_upstash:
                # Use Upstash Redis REST API
                logger.info("initializing_upstash_redis", url=settings.upstash_redis_rest_url)
                ...
                # Test connection
                try:
                    await self._client.set("connection_test", "ok", ex=5)
                    test_result = await self._client.get("connection_test")
                    await self._client.delete("connection_test")
                    logger.info("upstash_redis_connection_established", test_result=test_result)
                ...
                # Use regular Redis
                logger.info("initializing_regular_redis", url=self.redis_url[:50] + "...")
    ...
    async def close(self):
        ...
        if self._client and not self._use_upstash:
            # Only regular Redis clients need explicit closing
    ...
    async def get_search_results(self, cache_key: str) -> Optional[UnQuestResponse]:
        """
        ...
        Returns:
            Cached UnQuestResponse or None if not found
        """
        if not self._client:
            await self.initialize()
    ...
    def generate_cache_key(self, request: UnQuestRequest) -> str:
        """
        Generate deterministic cache key from request parameters.
        
        Args:
            request: UnQuestRequest object
    ...
    async def get_cached_url_content(self, url: str) -> Optional[str]:
        ...
        try:
            ...
            if content:
                if self._use_upstash:
                    # Upstash returns string values directly
                    return content
                else:
                    # Regular Redis returns bytes
    ...
    async def set_cached_url_content(
        ...
        try:
            if self._use_upstash:
                # Upstash REST API expects string values
                await self._client.setex(cache_key, ttl, content)
            else:
                # Regular Redis can handle bytes
