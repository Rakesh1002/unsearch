...
from app.services.extraction_strategies import create_extraction_strategy
from app.services.content_filters import create_content_filter
from app.services.markdown_generation import DefaultMarkdownGenerator
from app.services.adaptive_crawling import create_adaptive_crawler
from app.services.virtual_scrolling import PuppeteerVirtualScroller
from app.services.link_analysis import LinkAnalyzer, LinkPreviewConfig
from app.services.chunking_strategies import create_chunking_strategy
from app.services.table_extraction import create_table_extraction_strategy
from app.services.browser_config import BrowserConfig, create_browser_config_from_env
from app.services.dispatcher import create_dispatcher
from app.services.url_seeder import URLSeeder, SeedingConfig
...
class EnhancedScrapingService(ContentScrapingService):
    ...
    async def _scrape_with_virtual_scrolling(
        ...
        try:
            ...
            from app.services.virtual_scrolling import VirtualScrollConfig
    ...
    async def _analyze_links(
        ...
        try:
            ...
            from app.services.link_analysis import LinkPreviewConfig
            preview_config = LinkPreviewConfig(**link_config)
            
            # Initialize link analyzer
            if not self.link_analyzer:
                from app.services.link_analysis import LinkAnalyzer
    ...
    async def _adaptive_crawl_single_url(
        ...
        try:
            ...
            from app.services.adaptive_crawling import AdaptiveConfig
            
            crawl_config = AdaptiveConfig(**adaptive_config)
            
            # Create adaptive crawler
            if not self.adaptive_crawler:
                from app.services.adaptive_crawling import AdaptiveCrawler
