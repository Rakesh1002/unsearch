# SearchScrape API Operational Runbooks
...
**PagerDuty**: https://searchscrape.pagerduty.com  
**Status Page**: https://status.searchscrape.com  
**War Room**: Zoom: https://zoom.us/j/searchscrape-incident
...
docker ps | grep searchscrape-api
kubectl get pods -n production | grep api

# 2. Check recent logs
docker logs --tail 100 searchscrape-api
kubectl logs -n production deployment/api --tail=100

# 3. Check system resources
docker stats searchscrape-api
...
```bash
# 1. Check error logs
grep ERROR /var/log/searchscrape/api.log | tail -20
...
psql -c "REINDEX DATABASE searchscrape;"  # Rebuild indexes
...
docker logs searchscrape-searxng --tail 100
...
kubectl set image deployment/api api=searchscrape:$VERSION -n production
...
# 5. Full deployment
kubectl set image deployment/api api=searchscrape:$VERSION -n production --all

# 6. Verify deployment
./scripts/post-deploy-check.sh

# 7. Update status page
curl -X POST https://status.searchscrape.com/api/incidents \
...
docker build -t searchscrape:hotfix-$(date +%s) .
docker push searchscrape:hotfix-$(date +%s)

# 4. Deploy immediately
kubectl set image deployment/api api=searchscrape:hotfix-$(date +%s) -n production
...
curl http://api.searchscrape.com/health
```
...
logrotate -f /etc/logrotate.d/searchscrape

# Clean old logs
find /var/log/searchscrape -name "*.log.gz" -mtime +30 -delete

# Archive to S3
aws s3 sync /var/log/searchscrape s3://searchscrape-logs/$(date +%Y/%m)/
```
...
pg_restore --dbname=searchscrape \
...
# Network debugging
tcpdump -i any -w capture.pcap host api.searchscrape.com

# Process monitoring
htop -p $(pgrep -f searchscrape)
```
...
| ----------- | ------------------------------------ | -------------------------------------- | ---------------------------------------- |
| Production  | https://api.searchscrape.com         | https://admin.searchscrape.com         | https://grafana.searchscrape.com         |
| Staging     | https://staging-api.searchscrape.com | https://staging-admin.searchscrape.com | https://staging-grafana.searchscrape.com |
| Development | http://localhost:8000                | http://localhost:8000/admin            | http://localhost:3000                    |

### Support Resources

- **Documentation**: https://docs.searchscrape.com
- **API Status**: https://status.searchscrape.com
- **Internal Wiki**: https://wiki.searchscrape.internal
- **Slack**: #searchscrape-ops
- **JIRA**: https://searchscrape.atlassian.net
