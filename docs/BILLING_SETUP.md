...
This guide walks you through setting up the complete billing and authentication system for the SearchScrape API.
...
The SearchScrape API now includes a complete billing system with:
...
```bash
psql -h localhost -U searchscrape -d searchscrape -c "\dt"
```
