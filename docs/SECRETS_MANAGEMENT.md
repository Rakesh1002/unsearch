...
This guide provides comprehensive instructions for managing secrets, API keys, and sensitive configuration in the SearchScrape API system.
...
The SearchScrape API uses multiple layers of secret management to ensure security:
...
aws secretsmanager create-secret \
    --name searchscrape/production/api \
    --secret-string file://secrets.json

# Update secret
aws secretsmanager update-secret \
    --secret-id searchscrape/production/api \
    --secret-string file://secrets.json
```
...
def get_secrets():
    ...
    response = client.get_secret_value(SecretId='searchscrape/production/api')
  ...
  "Statement": [
    {
      ...
      "Resource": "arn:aws:secretsmanager:*:*:secret:searchscrape/*"
    }
  ]
}
```
...
vault secrets enable -path=searchscrape kv-v2

# Write secrets
vault kv put searchscrape/production \
...
psql -c "ALTER USER searchscrape WITH PASSWORD '$NEW_DB_PASS';"

# Update secret store
aws secretsmanager update-secret \
    --secret-id searchscrape/production/db \
    ...
    [
      {
        ...
        "Effect": "Allow",
        "Principal":
          { "AWS": "arn:aws:iam::123456789012:role/searchscrape-api" },
        "Action": ["secretsmanager:GetSecretValue"],
        "Resource": "arn:aws:secretsmanager:*:*:secret:searchscrape/production/*",
...
DATABASE_URL=postgresql://dev:dev@localhost:5432/searchscrape_dev
```
...
export SECRET_ARN=arn:aws:secretsmanager:us-east-1:123456789012:secret:searchscrape/staging
```
...
# Check secret store
aws secretsmanager get-secret-value --secret-id searchscrape/production
...
```bash
# Verify IAM role
aws sts assume-role --role-arn arn:aws:iam::123456789012:role/searchscrape-api

# Check policy
aws iam get-role-policy --role-name searchscrape-api --policy-name secrets-access
```
...
aws secretsmanager describe-secret --secret-id searchscrape/production

# View rotation history
aws secretsmanager list-secret-version-ids --secret-id searchscrape/production

# Manually trigger rotation
aws secretsmanager rotate-secret --secret-id searchscrape/production
```
...
kubectl rollout restart deployment/searchscrape-api
...
- Security Team: security@searchscrape.com
